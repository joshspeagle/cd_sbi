...
| **data_io** | Two data paths: (a) `aggregates.py` loads `index_row.parquet` from one or more run-dirs into tidy pandas frames (the existing `loaders.load_runs` pattern); (b) `checkpoints.py` loads `model.pt` and returns `{arch_metadata, final_loss, state_dict}` for figures that need model internals (C1, C3) or full `loss_history_tail` (E6, E7). Both paths resolve sweep paths via the manifest. | `data_io/aggregates.py`, `data_io/checkpoints.py` |
