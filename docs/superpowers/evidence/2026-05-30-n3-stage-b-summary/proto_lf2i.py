"""LF2I-BFF baseline on (μ,Σ) d=5 from RAW X∈ℝ²⁰. Tests the user's hypothesis: a
2-step calibration method should get VALID frequentist coverage even without a
sufficient summary. Compare coverage_error to CD-SBI Stage-A (~0.026). Caveat:
BFF integration grid uses a scalar bounding box (-3,3)^5 (sim has per-dim ranges);
acceptable for a 'how does it compare' baseline (out-of-prior cov grid pts → ~0 odds)."""
import numpy as np
import torch
...
print(f"trained LF2I-BFF (stage1 final loss {trained.final_loss:.3f}, wall {trained.wall_clock_sec:.0f}s)\n")

# Coverage on the 3-pt grid (center + 2 extremes) used for CD-SBI per-coord PIT.
...
print(f"\ncoverage_error_max (3-pt grid) = {worst:.3f}")

# Broader LHS grid (16 pts over the true per-dim prior) for a fairer coverage_error_max.
...
print(f"coverage_error_max (16-pt LHS over true prior) = {worst_lhs:.3f}")
print("\nCompare: CD-SBI Stage-A (oracle Bartlett) coverage ~0.026; structure-summary calibrates similarly.")
