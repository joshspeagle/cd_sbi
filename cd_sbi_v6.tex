...
to the synthesis in Schweder \& Hjort (2016). The defining property is
\textbf{exact coverage}: \[
H(\theta_0; X) \;\sim\; U(0,1) \quad \text{when } X \sim P_{\theta_0}, \text{ for every } \theta_0 \in \Theta.
\] Equivalently: \(H(\theta_0; X)\) is the \emph{probability integral
transform} (PIT) --- the mapping that converts any continuous random
variable into a uniform \((0,1)\) variable via its own CDF --- applied
to a correctly chosen \emph{pivot}.
...
In regular one-parameter models the optimal (uniformly most powerful
unbiased, ``UMPU'') CD is explicit and unique. For
\(X \sim \mathcal{N}(\theta, 1)\) it is \[
H^*(\theta; X) = \Phi(\theta - X),
...
\item
  \textbf{(C2) Monotonicity in \(\theta\).} For each fixed \(X\) and
  each coordinate \(k\), \(\theta_k \mapsto r_k(\theta; X)\) is strictly
  increasing (with autoregressive conditioning in \(d > 1\); see \S\ref{sec:6}).
  This ensures connected level sets, so confidence regions are
  well-defined geometric objects.
\end{itemize}
...
limit the CD pivot does not depend on \(\rho\), provided \(\rho\) has
full support on the region of inferential interest.
...
\tightlist
\item
  \textbf{(R1) Monotonicity in \(\theta\).} For
  \(\rho \otimes P_\theta\)-a.e. \(X\) and each coordinate \(k\), the
  map \(\theta_k \mapsto r_k(\theta; X)\) is strictly increasing when
  other components are held fixed (autoregressively in \(d > 1\); see
  \S\ref{sec:6}). In 1D this is just: \(\theta \mapsto r(\theta; X)\) is strictly
  increasing.
\item
  \textbf{(R2) Invertibility in \(X\).} For each fixed \(\theta\), the
  map \(X \mapsto r(\theta; X)\) is a \(C^1\) (or Lipschitz, a.e.
  differentiable) diffeomorphism onto its image. Equivalently,
  \(\partial r / \partial X\) has full rank a.e.
\end{itemize}
...
Without (R1), the calibration manifold has a \(\mathbb{Z}_2^d\)
ambiguity (sign flips in each coordinate of \(r\)), and the induced
``confidence sets'' can be disconnected. (R1) eliminates this.
...
In \S\ref{sec:3} we will see that NF-MLE applied to a non-bijective \(r\) has
\emph{spurious} sub-optima below the entropy of the true model --- the
model gains apparent likelihood by folding \(X\)-space, inflating
\(|\det \partial r / \partial X|\) in regions where preimages collide.
Note that under folding, \(\hat p_r\) does \emph{not} integrate to one
in \(X\) at fixed \(\theta\), so there is no lower bound on
\(-\log \hat p_r\) from the true conditional entropy; the loss can dip
arbitrarily far below the true model's NF-MLE value. The empirical
demonstration is in \S\ref{subsec:8.4}. Enforcing (R2) architecturally fixes this.
...
\emph{Proof.} Suppose for contradiction \(\psi'\) changes sign. By
continuity of \(\psi'\), there is some \(X_* \in \mathbb{R}\) with
\(\psi'(X_*) = 0\) and (WLOG, taking a local maximum) values
\(X_1, X_2\) arbitrarily close to \(X_*\) with \(X_1 < X_* < X_2\) such
that \(\psi(X_1) = \psi(X_2) = v < v_* := \psi(X_*)\), with \(\psi\)
strictly monotone on each of the intervals \((X_1, X_*)\) and
\((X_*, X_2)\) for \(v\) close enough to \(v_*\).

The pushforward density of \(\mathcal{N}(\theta_0, 1)\) under \(\psi\)
at value \(v\) in the range of \(\psi\) where the preimage is finite is
\[
g_\psi(v) \;=\; \sum_{i\,:\,\psi(X_i)=v} \frac{\varphi(X_i - \theta_0)}{|\psi'(X_i)|}.
\] For \(v\) just below \(v_*\), summing over the two preimages
\(X_1(v) \to X_*^-\) and \(X_2(v) \to X_*^+\), \[
g_\psi(v) \;\geq\; \frac{\varphi(X_1(v) - \theta_0)}{|\psi'(X_1(v))|} + \frac{\varphi(X_2(v) - \theta_0)}{|\psi'(X_2(v))|}.
\] By Taylor expansion at \(X_*\) (using \(\psi'(X_*) = 0\) and
\(\psi''(X_*) < 0\) at a local max, or more generally the leading
nonzero derivative), \(|\psi'(X_i(v))| = O(|v - v_*|^{1/2})\) for a
quadratic critical point, while
\(\varphi(X_i(v) - \theta_0) \to \varphi(X_* - \theta_0) > 0\). So
\(g_\psi(v) \to \infty\) as \(v \to v_*^-\). But the target density
\(\varphi(v_*)\) is finite. Contradiction. (For higher-order critical
points the divergence is faster; the conclusion is unchanged.)
\(\blacksquare\)
...
\emph{Selecting the direction.} The case
\(\psi_{\theta_0}(X) = X - \theta_0\) for \(\rho\)-a.e. \(\theta_0\)
gives \(r(\theta, X) = X - \theta\), which has
\(\partial_\theta r = -1\) --- contradicting (R1). Hence the other
branch: \(\psi_{\theta_0}(X) = \theta_0 - X\), giving
\(r(\theta, X) = \theta - X\), with \(\partial_\theta r = +1 > 0\)
satisfying (R1). \(\blacksquare\)
...
strict monotone in \(U\), plus a new condition:
...
\textbf{Why (R4) is automatic in the continuous case.} Lemma 4.2 (or
(R3)) implies \(\psi_{\theta_0}\) is monotone in \(X\), and by
sufficiency monotone in \(T(X)\). Only the discrete case has the extra
freedom that requires (R4) as a separate condition.
...
Theorem A* requires (R3) as an
extra assumption. Whether (R3) follows from (R1) + Lipschitz +
calibration alone is open. Lemma 4.3 gives first-order rigidity, so any
failure mode is isolated; in practice, architectural enforcement makes
the question moot.
