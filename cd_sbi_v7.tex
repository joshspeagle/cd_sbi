...
Trust Crisis in Simulation-Based Inference?'') documented that all four
major SBI families systematically produce \emph{overconfident}
posteriors at realistic budgets --- credible intervals have empirical
coverage well below nominal. Subsequent work has tried to retrofit
...
rearrangement} \citep{Rosenblatt1952} is the unique transport map of
...
\textbf{Definition (UMNN; \citealp{WehenkelLouppe2019}).} A
...
entropic/diagonal-scaling continuation, raising the
...
is open. \citet{WehenkelEtAl2025} study a related question via a
reweighted / generalized-Bayes robust SBI objective; a direct
coverage-degradation analysis of the forward-KL projection of \(p\)
onto \(\mathcal{F}\) is, to our knowledge, still missing.
Robust-SBI ideas \citep{SchmonEtAl2020, DellaportaEtAl2022} should also
integrate with CD-SBI but the right framework is unclear.
...
dimension. Autoregressive flows scale routinely to \(d \sim 30\) in the SBI
literature \citep{LueckmannEtAl2021} and to \(d \sim 100\)+ in general
normalizing-flow contexts \citep{PapamakariosEtAl2017, DurkanEtAl2019}.
What is specific to CD-SBI is the requirement
...
\item \emph{Naive sequential CD-SBI (adapting \(\rho\) to concentrate
  ...
  argument of \S\ref{subsec:1.3}, and the broader sequential-SBI
  literature \citep{HermansEtAl2022, LueckmannEtAl2021} has documented
  empirical miscalibration of all naive sequential variants.
